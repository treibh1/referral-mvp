#!/usr/bin/env python3
"""
Full location enrichment script to run Brave API location search on all contacts.
...
def enrich_contacts_batch(df, start_idx, end_idx, batch_name):
    """
    Enrich a batch of contacts with location data.
    """
    ...
    for i in range(start_idx, min(end_idx, len(df))):
        ...
        try:
            # Extract location using Brave API
            ...
            if location_info and location_info.get('location'):
                ...
                df.at[i, 'location_source'] = location_info.get('source', 'Brave Search API (Google)')
...
def main():
    """Main function to run full location enrichment."""
    print("🚀 Starting Full Location Enrichment Process")
    print("=" * 60)
    ...
    print(f"📊 Will process in {total_batches} batches of {batch_size} contacts each")
    ...
    for batch_num in range(total_batches):
        ...
        batch_name = f"{batch_num+1:03d}_of_{total_batches:03d}"
        
        print(f"\n🎯 Processing Batch {batch_num+1}/{total_batches}")
        
        successful, failed, skipped = enrich_contacts_batch(df, start_idx, end_idx, batch_name)
    ...
    print(f"\n🎯 Full Location Enrichment Complete!")
    print(f"=" * 60)
